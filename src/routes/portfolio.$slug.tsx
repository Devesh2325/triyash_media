...
export const Route = createFileRoute("/portfolio/$slug")({
      ...
      scripts: [
        {
          ...
          children: JSON.stringify({
            ...
            image: loaderData.gallery.map((item) => (typeof item === "string" ? item : item.poster ?? item.src)),
  ...
  notFoundComponent: () => (
    <div className="section container-luxe text-center">
      ...
      <Link to="/portfolio" className="mt-6 inline-block text-gold underline">
        Back to portfolio
      </Link>
    </div>
  ),
    ...
    <>
      <section className="relative isolate overflow-hidden">
        ...
        <div className="container-luxe pt-32 pb-28 md:pt-40 md:pb-36 text-[var(--cream)]">
          <Link to="/portfolio" className="eyebrow text-gold hover:opacity-80">
            ← Portfolio
          </Link>
        ...
        <div className="container-luxe">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {c.gallery.map((item, i) => {
              ...
              const label = typeof item === "string" ? `${c.title} — ${i + 1}` : item.label;

              return (
              <div
                key={source}
                className={`overflow-hidden rounded-3xl ${i === 0 ? "md:col-span-2 aspect-[16/8]" : "aspect-[4/3]"}`}
              >
                {isVideo ? (
                  <video
                    controls
      ...
      <section className="section pt-0">
        <div className="container-luxe">
          <div className="flex items-end justify-between gap-6 mb-10">
            ...
            <Link to="/portfolio" className="text-sm text-gold hover:underline">
              All projects →
            </Link>
          </div>
